...
const VAULT_PRESETS = ['aave-v3-erc4626-vault', 'morpho-blue-vault', 'compound-v3-vault'] as const;
...
server.registerTool(
  'harness_audit',
  {
    ...
    description:
      'Audit Solidity against a corpus of documented DeFi integration findings: ERC-4626 ' +
...
server.registerTool(
  'harness_generate',
  {
    ...
    description:
      'Generate a DeFi contract that is already hardened against the documented findings — ' +
    ...
    inputSchema: {
      ...
      asset: z
        ...
        .optional()
        .describe(
          'Underlying ERC20 address (vaults and the flash-loan receiver). Defaults to mainnet USDC. ' +
  ...
  async (args) => {
    const r = await callApi<{
      ...
      propertyTestSource: string;
      deployScriptSource: string;
      appliedFindingIds: string[];
      testNames: string[];
      propertyTests: { name: string; claim: string }[];
    }>('/api/generate', args);
